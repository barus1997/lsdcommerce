<?php 
/**
 * Rendering Shipping based on Product
 * 
 * @package LSDDonation
 * @subpackage Render
 * @since    1.0.0
 */
add_action( 'lsdcommerce_checkout_shipping', 'lsdc_shipping_method' );
function lsdc_shipping_method(){
...
/**
 * Prcossing Extra Payment
 * Call : First Init Checkout
 * Call : Update Shipping Cost
 * Call : Add Coupon
 */
add_action( 'wp_ajax_nopriv_lsdc_checkout_extra_processing', 'lsdc_checkout_extra_processing' );
add_action( 'wp_ajax_lsdc_checkout_extra_processing', 'lsdc_checkout_extra_processing' );
function lsdc_checkout_extra_processing(){
    $extras = array();
...
/**
 * Calculate :: Shipping Package
 * When :: On Change City
 * Test :: Tested 5 Sept 2020
 */
add_action( 'wp_ajax_nopriv_lsdc_shipping_physical_package', 'lsdc_shipping_physical_package' );
add_action( 'wp_ajax_lsdc_shipping_physical_package', 'lsdc_shipping_physical_package' );
function lsdc_shipping_physical_package(){
    global $lsdcommerce_shippings;
    if ( ! check_ajax_referer( 'lsdc_nonce', 'security' ) )  wp_send_json_error( 'Invalid security token sent.' );
    if ( ! wp_verify_nonce( $_REQUEST['nonce'], 'checkout-nonce' ) ) { die('Busted'); }
 ...
 * When :: On Change Password
 * Test :: Tested 3 Sep 2020
 */
add_action( 'wp_ajax_nopriv_lsdc_member_profile_password', 'lsdc_member_profile_password' );
add_action( 'wp_ajax_lsdc_member_profile_password', 'lsdc_member_profile_password' );
function lsdc_member_profile_password(){
    if ( ! check_ajax_referer( 'lsdc_nonce', 'security' ) )  wp_send_json_error( 'Invalid security token sent.' );
