 ...
 * Set order counter
 */
function lsdc_order_counter_add(){
    $lsdc_order_counter = get_option('lsdc_order_counter');
	if( is_numeric( $lsdc_order_counter ) ) {
		$lsdc_order_counter++;
		update_option('lsdc_order_counter', $lsdc_order_counter);
	}else{
 ...
 * @param boolean $block
 */
function lsdc_order_status( $order_id, $state, $block = false )
{
    // Blocking Notification
    // if( $block == false ){

        switch ($state) {
            case 'new': // Pesanan  Baru
                wp_schedule_single_event( time() + 3, 'lsdc_notification_schedule', array( $order_id, 'order' ) ); // Kirimg Notifikasi Order Masuk
                break;

            case 'canceled': // Notifikasi Pemabatalan
                wp_schedule_single_event( time() + 3, 'lsdc_notification_schedule', array( $order_id, 'canceled' ) ); 
                break;

            case 'paid': // Pesnaan Dibayar
            ...
            case 'shipped': // Dalam Pengiriman
                wp_schedule_single_event( time() + 9, 'lsdc_notification_schedule', array( $order_id, 'shipped' ) ); 
                break;

            case 'completed': // Pesanan Selesai
                wp_schedule_single_event( time() + 10, 'lsdc_notification_schedule', array( $order_id, 'completed' ) ); 
                break;
        }

    // }
    update_post_meta( $order_id, 'status', $state );
}
